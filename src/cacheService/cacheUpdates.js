...
export const addNewTask = (addedTask) => {
  ...
  const newTicketOrder = ticketOrder.concat({
    ticketId: addedTask.id,
    type: "task",
  });
  client.writeFragment({
    id: columnIdForCache,
...
export const removeBoardFromCache = (boardId, projectId) => {
  const boardToBeDeleted = `Board:${boardId}`;
  const projectIdForCache = `Project:${projectId}`;
  const { boards } = client.readFragment({
    id: projectIdForCache,
    fragment: PROJECTS_BOARDS,
...
export const removeTaskFromCache = (taskId, columnId, boardId) => {
  ...
  const newTicketOrder = ticketOrder.filter(
    (taskObj) => taskObj.ticketId !== taskId
  );
...
export const cacheTicketMovedFromColumn = (
  ...
  if (ticketInfo.type === "task") {
    const newSourceTasks = Array.from(sourceColumn.tasks).filter(
      (task) => task.id !== ticketInfo.ticketId
    );
    const movedTicket = sourceColumn.tasks.find(
      (task) => task.id === ticketInfo.ticketId
    );
    const newDestinationTasks = Array.from(destinationColumn.tasks).concat(
      movedTicket
    );
    client.writeFragment({
      id: sourceColumnIdForCache,
    ...
    const newSourceSubtasks = Array.from(sourceColumn.subtasks).filter(
      (subtask) => subtask.id !== ticketInfo.ticketId
    );
    const movedTicket = sourceColumn.subtasks.find(
      (subtask) => subtask.id === ticketInfo.ticketId
    );
    const newDestinationSubtasks = Array.from(
