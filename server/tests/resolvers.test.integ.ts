...
import {board1, column1, column2, task1} from "./dummyData"
...
describe('mutations', () => {
    ...
    test('should be able to move a task from column to another', async()=>{
        const column = await ColumnQuery.columnById(null, column1)
        const secondColumn = await ColumnQuery.columnById(null, column2)
        await ColumnMutation.moveTicketFromColumn(null, {
            type: "task",
            ticketId: "f3f3c12d-cee0-46bf-9374-f19ba8894ad6",
            sourceColumnId: column.dataValues.id,
            destColumnId: secondColumn.dataValues.id,
            sourceTicketOrder: [],
            destTicketOrder: [{ticketId:"f3f3c12d-cee0-46bf-9374-f19ba8894ad6",type: "task"}],
            eventId: ""
